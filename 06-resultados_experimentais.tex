...
As tabelas que seguem demonstram respectivamente: o conjuntos de experimentos na versão original e suas adaptações na Tabela \ref{tab:metrics_acdc_orig}, experimentos coma a adição das máscaras como terceiro vetor de características na Tabela \ref{tab:metrics_acdc_orig_mask} e, por fim, a versão que adiciona os blocos convolucionais e \gls{SE} na Tabela \ref{tab:metrics_acdc_se}. Alguns pontos podem ser notados no experimento, o treinamento quase sempre sofre sobre-ajuste ficando muito próximo de 100\% de assertividade, isso pode se justificado dado o fato de haver poucos exemplares para treinamento. Outro ponto notado é que quanto maior o tamanho do vetor de características (Emb) melhor costuma ser o resultado. Quantidades maiores no múmero de blocos de autoatenção (N\_Attn) também melhoram os resultados, exceto no caso em que são configurados em $6$. A justificativa se deve ao fato de tornar o modelo mais complexos se comparados a quantidade de dados para treino, prejudicando a sua capacidade de generalização. Os melhores resultados são conferidos nas versões que adicionam o bloco \gls{SE}, sendo que o melhor experimento obteve $78\%$ de assertividade e aparece em azul. A visualização dos modelos com bloco \gls{SE}, coletados no \textit{CometML} é visto na Figura \ref{fig:fig032}.
...
Outras adaptações mais simplórias também surtiram efeito, como aumentar o tamanho das características utilizadas, aplicar mais blocos de autoatenção, a adição das máscaras, etc. Os experimentos indicam que há espaço para exploração e melhorias, como por exemplo, outros mecanismos de atenção, podem trazer resultados ainda mais promissores para o âmbito de cardiomiopatia hipertrófica. 
